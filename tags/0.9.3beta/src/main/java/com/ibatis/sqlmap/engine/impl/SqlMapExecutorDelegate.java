...
public class SqlMapExecutorDelegate {
  ...
  private HashMap shardingMaps;//sean add
  ...
  public SqlMapExecutorDelegate() {
    ...
    shardingMaps = new HashMap(); //sean add
  ...
  /**
   * Add Sharding config,table name to lower case.
   * @param sc
   */
  public void addShardingConfig(ShardingConfig sc){
	  if(sc == null){
		  throw new IllegalArgumentException("The parameter ShardingConfig must not null.");
	  }
	  String key=sc.getTableName().toLowerCase();
	  if(shardingMaps.containsKey(key)){
		  throw new SqlMapException("There is already a sharding config for table " + key + " .");
	  }
	  shardingMaps.put(key, sc);
  }
  
  /**
   * Get Sharding config,tableName ignore case.
   * @param tableName
   * @return
   */
  public ShardingConfig getShardingConfig(String tableName){
	  ShardingConfig sc = (ShardingConfig) shardingMaps.get(tableName.toLowerCase());
	  return sc;
  }
