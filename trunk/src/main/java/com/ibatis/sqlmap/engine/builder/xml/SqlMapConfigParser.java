...
public class SqlMapConfigParser {
  ...
  private void addSettingsNodelets() {
    parser.addNodelet("/sqlMapConfig/settings", new Nodelet() {
  	 ...
  	 * @author Sean
  	 */
	private void addShardingNodelets() {
		parser.addNodelet("/sqlMapConfig/shardingConfig/sharding", new Nodelet() {
			public void process(Node node) throws Exception {
				...
				String strategyClass = attributes.getProperty("strategyClass");
				strategyClass = strategyClass == null ? "com.ibatis.sqlmap.engine.sharding.impl.DefaultShardingStrategy"
						: strategyClass;
				ShardingConfig sc = new ShardingConfig();
				sc.setTableName(tableName);
				Class strategy=Class.forName(strategyClass);
				ShardingStrategy obj=(ShardingStrategy)strategy.newInstance();
				sc.setStrategy(obj);
				
				config.getClient().getDelegate().addShardingConfig(sc);
			}
		});
	}
